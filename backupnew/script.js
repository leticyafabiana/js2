...
function displayNews(articles) {
    ...
    articles.forEach(article => {
        ...
        newsItem.innerHTML = `
            ...
            <button class="audio-button" onclick="readNews('${article.title}', '${article.description}')">Ouvir Notícia</button>
            <button class="audio-button" onclick="togglePause()">Pausar</button>
            <button class="favorite-button" onclick="saveFavorite('${article.title}', '${article.description}', '${article.urlToImage}', '${article.url}')">Salvar como Favorito</button>
        `;
        
        newsContainer.appendChild(newsItem);
    });
}
function saveFavorite(title, description, urlToImage, url) {
    // Obter favoritos existentes do localStorage
    let favorites = JSON.parse(localStorage.getItem('favorites')) || [];

    // Cria um objeto de notícia com as informações
    const newsItem = {
        title,
        description,
        urlToImage,
    ...
    // Verificar se a notícia já está nos favoritos
    const isFavorite = favorites.some(item => item.title === title);
    if (!isFavorite) {
        favorites.push(newsItem);
        localStorage.setItem('favorites', JSON.stringify(favorites));
        alert('Notícia salva como favorita!');
    } else {
        alert('Esta notícia já está nos favoritos.');
    }
}
function displayFavorites() {
    const favoritesNewsContainer = document.getElementById('favorites-news');
    const favorites = JSON.parse(localStorage.getItem('favorites')) || [];

    favoritesNewsContainer.innerHTML = '';

    if (favorites.length === 0) {
        favoritesNewsContainer.innerHTML = '<p>Nenhuma notícia salva como favorita.</p>';
    } else {
        favorites.forEach(article => {
            ...
            favoritesNewsContainer.appendChild(newsItem);
        });
    }
}
document.addEventListener('DOMContentLoaded', function() {
    fetchNews(); // Carregar notícias inicialmente
    displayFavorites(); // Carregar favoritos inicialmente
});
    ...
    if (favorites.length === 0) {
        ...
        favorites.forEach(article => {
            ...
            newsItem.innerHTML = `
                ...
                <p>${article.description || 'Descrição não disponível'}</p>
                <a href="${article.url}" target="_blank">Leia mais</a>
                <button class="remove-favorite-button" onclick="removeFavorite('${article.title}')">Remover Favorito</button>
            `;
...
function removeFavorite(title) {
    ...
    localStorage.setItem('favorites', JSON.stringify(favorites));
    displayFavorites(); // Atualizar a exibição dos favoritos
}
function displayNews(articles) {
    const newsContainer = document.getElementById('news-container');
    newsContainer.innerHTML = '';
    ...
    articles.forEach(article => {
        ...
        // Defina uma imagem padrão caso a notícia não tenha uma imagem
        const imageUrl = article.urlToImage || 'padrão.webp';

        newsItem.innerHTML = `
            <h2>${article.title}</h2>
            <img src="${imageUrl}" alt="Imagem da Notícia" style="width:100%;">
            <p>${article.description || 'Descrição não disponível'}</p>
            <a href="${article.url}" target="_blank">Leia mais</a>
            <button class="audio-button" onclick="readNews('${article.title}', '${article.description}')">Ouvir Notícia</button>
            <button class="audio-button" onclick="togglePause()">Pausar</button>
            <button class="favorite-button" onclick="saveFavorite('${article.title}', '${article.description}', '${imageUrl}', '${article.url}')">Salvar como Favorito</button>
        `;
